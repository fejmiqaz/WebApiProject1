...
namespace KursiWebApi.Controllers
{
    ...
    public class LendetController : ControllerBase
    {
        ...
        {
            return await _repository.GetAll().Include(f => f.Klasa).Include(f => f.Studentet).FirstOrDefaultAsync(lenda => lenda.Id == id, token);
            
        }

        [HttpPost]
        [Route("[action]")]
        public async Task<IActionResult> Regjistro(LendaDto lenda, CancellationToken token)//spjego cancelation token
        {
        ...
        [HttpPost]
        [Route("[action]")]
        public string Login(AuthRequestsObject authObj, CancellationToken cancellationtoken)
        {
            if(authObj.Username == "femky" && authObj.Password == "qazimi")
            {
